...
class Geo extends Base
{
    ...
    private static function tzdata($id)
    {
        ...
        return array(
            'offset' => $ref->getOffset(new DateTime('now', new DateTimeZone('GMT'))) / 3600,
    ...
    static function  geo_ip($ip, $is_rf)
    {
        ...
        $city = DB::sql('EXEC usp_RegionGetByName @RegionName=:RegionName', array(
            ':RegionName' => array($city, PDO::PARAM_STR)
        ));

        if ($city[0]) {
            $sub_d = $is_rf ? $city[0]['DomenRus'] : $city[0]['DomenEn'];
            $city_name = $city[0]['RegionName'];
            $city_id = $city[0]['RegionID'];
        } else {
            $sub_d = $is_rf ? 'ирк' : 'irk';
            $city_name = 'г. Иркутск';
            $city_id = 1;
        ...
        setcookie('cityinfo', $city_name . '#' . $city_id, time() + 3600 * 24 * 365, '/', '.' . F3::get('host'));
        $rerout = 'http://' . $sub_d . '.' . F3::get('host') . '/';
        return $rerout;
    }
