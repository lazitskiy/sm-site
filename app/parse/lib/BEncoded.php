...
class BEncoded
{
    ...
    {
        return $this->Alter($name, 'set', $value);
    }

    function Copy($src, $dest)
    {
        return $this->Set($dest, $this->ValueOf($src));
    }
    ...
    {
        return $this->Dump($this->ValueOf($name));
    }
    ...
    function NewNode($type, $name)
    {
        switch ($type = strtolower($type)) {
            ...
            case 'dict':
                return $this->Set($name, array('isDct' => true));
            default:
                throw new EBEncode($this, 'Cannot create bencoded node because type ' . $type . ' is unknown.');
        }
    }
            ...
            if ($type === 'int' or $type === 'str') {
                switch ($asType) {
                    ...
                    case 'list':
                        ...
                        break;
                    case 'dict':
                        $value = array('isDct' => true, 0 => $value);
                        break;
                }
