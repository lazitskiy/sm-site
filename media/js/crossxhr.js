...
////////////////
  ...
  //var SWF_URL = "localhost/dle92/templates/kino_tt/js/crossxhr.swf"; // set if you want to hardcode path to crossxhr.swf
...
$(function() {
		...
		if (request.readyState == 4) {
			try {
				if (request.status == 200) {
					var kp_rating = request.responseText.match(
						/<kp_rating num_vote='(\d*)'>([\d\.]*)<\/kp_rating>/);
					var imdb_rating = request.responseText.match(
						/<imdb_rating num_vote='(\d*)'>([\d\.]*)<\/imdb_rating>/);
					$('#kinipoisk .kinorating').html(
						'<div class="kp-rating1"><span>' + kp_rating[2] + '</span></div>' +
						'<div class="kp-rating2"><span>' + imdb_rating[2] + '</span></div>'
					);
				} else {
					$('#kinipoisk .kinorating').html(
						'Load error'
	...
	var filmId = parseInt($('#kinipoisk .kinoid').html());
	request.open('GET', 'http://www.kinopoisk.ru/rating/' + filmId +'.xml');
	request.send();
});